...
class TransportLayer:
    ...
    def _update_jitter(self, pkt: Packet) -> Optional[float]:
        try:
            ...
            jlist = self._jitter_per_src.setdefault(src, [])
            jlist.append(jitter)
            if len(jlist) > 500:
                del jlist[:-500]
    ...
    def _start_rto(self, packet: Packet) -> None:
        ...
        tp_seq = int(tp_seq)
        now = float(self.sim.engine.now)
        if len(self._unacked) >= self.max_pending:
            oldest = min(self._unacked, key=lambda k: self._unacked[k][0])
            self._unacked.pop(oldest, None)
