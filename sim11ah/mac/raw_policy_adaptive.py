...
class AdaptiveRawPolicy:
    ...
    def __init__(self, ctx, log_fn) -> None:
        ...
        # ------------------------------------------------------------------
        # Beacon-interval time budget: Ng * Ns * slot_duration must fit
        # within budget_fraction of the beacon interval, or the RAW
        # schedule overruns past the next beacon (standard-noncompliant).
        # Fixed group/slot count means this bounds slot duration directly.
        # ------------------------------------------------------------------
        self.budget_fraction = float(self.mac_cfg.get("adaptive_raw_budget_fraction", 0.90))
        ...
        self._last_rx_count: Dict[int, int] = {}
    ...
    def _estimate_sta_observed_load(self, aid: int) -> float:
        """
        Per-STA demand observation: number of DATA frames the AP has
        successfully received from this AID since the last prediction
        update. Derived purely from local receptions (no control-packet
        overhead, no cross-node introspection).
        """
        rx_by_src = getattr(self.ctx, "_rx_count_by_src", None) or {}
        current = int(rx_by_src.get(aid, 0))
        last = self._last_rx_count.get(aid, current)
        self._last_rx_count[aid] = current
        return float(max(0, current - last))
    ...
    def _apply_safety_lower_bound(self, slot_us: int) -> int:
        ...
        cw_min = float(self.mac_cfg.get("cw_min", 15))
        sigma = self._sigma_s()
        # Use full cw_min (worst-case backoff) for a safety lower bound, not cw_min/2 (average).
        worst_case_backoff_s = cw_min * sigma

        safe_min_s = ts + raw_guard + ack_guard + worst_case_backoff_s
