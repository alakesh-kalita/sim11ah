...
from sim11ah.mac.raw_policy_cluster_csv import ClusterCsvAdaptiveRawPolicy
from sim11ah.mac.raw_policy_cluster_adaptive import ClusterAdaptiveRawPolicy
from sim11ah.mac.raw_policy_traffic_split import TrafficSplitRawPolicy
from sim11ah.mac.raw_policy_traffic_aware import TrafficAwareRawPolicy
from sim11ah.mac.raw_policy_laca import LacaRawPolicy
from sim11ah.mac.raw_policy_chang2019 import ChangGroupingRawPolicy
from sim11ah.mac.raw_policy_etaroa import ETaroaRawPolicy
from sim11ah.mac.raw_RL_policy import RLRawPolicy
...
class RawEngine:
    ...
    def _build_policy(self):
        ...
        if mode == "cluster_csv":
            self._log("RAW_POLICY_SELECT", {"policy": "cluster_csv"})
            return ClusterCsvAdaptiveRawPolicy(self.ctx, self._log)

        if mode == "cluster_adaptive":
            self._log("RAW_POLICY_SELECT", {"policy": "cluster_adaptive"})
            return ClusterAdaptiveRawPolicy(self.ctx, self._log)
        ...
        if mode == "traffic_split":
            self._log("RAW_POLICY_SELECT", {"policy": "traffic_split"})
            return TrafficSplitRawPolicy(self.ctx, self._log)

        if mode == "traffic_aware":
            self._log("RAW_POLICY_SELECT", {"policy": "traffic_aware"})
            return TrafficAwareRawPolicy(self.ctx, self._log)

        if mode == "laca":
            self._log("RAW_POLICY_SELECT", {"policy": "laca"})
            return LacaRawPolicy(self.ctx, self._log)

        if mode == "chang2019":
            self._log("RAW_POLICY_SELECT", {"policy": "chang2019"})
            return ChangGroupingRawPolicy(self.ctx, self._log)

        if mode == "etaroa":
            self._log("RAW_POLICY_SELECT", {"policy": "etaroa"})
            return ETaroaRawPolicy(self.ctx, self._log)

        if mode == "rl":
            self._log("RAW_POLICY_SELECT", {"policy": "rl"})
            return RLRawPolicy(self.ctx, self._log)
    ...
    def apply_rps(self, rps: List[Dict[str, Any]], raw_guard: float) -> None:
        ...
        all_candidates: List[Tuple[int, float, float, int, Dict[str, Any]]] = []
        ...
        for group in rps:
            ...
            all_candidates.append((slot_idx, enter_t, exit_t, gen, group))

        # A STA may legitimately match more than one RPS Assignment subfield
        # within the same beacon interval (e.g., TRAW's Mechanism B repeat
        # window covers the same AID range as the primary window, at a later
        # offset). Sort by start time and greedily keep all non-overlapping
        # windows so the STA is scheduled into every RAW opportunity it is
        # assigned, not just the earliest one.
        all_candidates.sort(key=lambda c: c[1])
        candidates: List[Tuple[int, float, float, int, Dict[str, Any]]] = []
        for cand in all_candidates:
            if candidates and cand[1] < candidates[-1][2]:
                continue
            candidates.append(cand)

        self.ctx._raw_candidates = candidates
        self.ctx._raw_candidate_idx = 0
        ...
        if chosen is None:
            ...
            if self.ctx.state != MacState.WAIT_ACK and self.ctx._assoc_state == AssocState.ASSOCIATED:
                self.ctx.state = MacState.RAW_SLEEP
                self._inc_stat("raw_sleep_transitions")
                _e = getattr(self.ctx.sim, "energy", None)
                if _e is not None:
                    try:
                        _e.on_sleep_start(self.ctx.node.node_id, self.ctx.sim.engine.now)
                    except Exception:
                        pass
        ...
        self._schedule_raw_window(chosen, aid)

    def _schedule_raw_window(
        self,
        chosen: Tuple[int, float, float, int, Dict[str, Any]],
        aid: int,
    ) -> None:
    ...
    def raw_enter(self, gen: int) -> None:
        ...
        if self.ctx.state != MacState.WAIT_ACK:
            ...
            # Energy: close any open sleep window when the node wakes for its slot.
            _e = getattr(self.ctx.sim, "energy", None)
            if _e is not None:
                try:
                    _e.on_sleep_end(self.ctx.node.node_id, self.ctx.sim.engine.now)
                except Exception:
                    pass
    ...
    def raw_exit(self, gen: int) -> None:
        ...
        # If this STA was assigned more than one non-overlapping RAW window
        # in this beacon interval (e.g., TRAW Mechanism B repeat window),
        # schedule the next one now instead of sleeping until the next
        # beacon.
        candidates = getattr(self.ctx, "_raw_candidates", None)
        idx = getattr(self.ctx, "_raw_candidate_idx", 0)
        if candidates and idx + 1 < len(candidates):
            self.ctx._raw_candidate_idx = idx + 1
            next_chosen = candidates[idx + 1]
            aid = self._get_self_aid()
            self._schedule_raw_window(next_chosen, aid)
            return
    ...
    def exchange_would_fit_in_raw(
        ...
        *,
        ...
        full_budget = float(rts_overhead_s) + t_data + prop + self.ctx.sifs + t_ack + prop + self.ctx.ack_guard
        eps = max(1e-6, float(getattr(self.ctx, "ack_guard", 0.0)) * 0.25)

        if cross_slot:
            # Data may overflow into the next slot, but the full DATA+ACK exchange
            # must complete within remaining + next slot. Guard time is excluded
            # from next-slot budget because the AP needs it for overhead.
            next_slot_budget = (
                float(current_group.get("slot_duration", self.ctx.raw_slot_duration))
                - float(self.ctx.raw_guard)
            )
            needed = full_budget
            ok = needed <= (remaining + next_slot_budget + eps)
            reason = "cross_slot_full_budget"
        else:
            needed = full_budget
