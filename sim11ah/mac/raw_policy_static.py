...
class StaticRawPolicy:
    ...
    - validates total RAW time against the available beacon RAW budget
    """
    ...
    def _get_raw_budget_info(self) -> Dict[str, int]:
        mac_cfg = self.ctx.cfg["mac"]

        beacon_interval_us = int(float(mac_cfg.get("beacon_interval", 0.5)) * 1e6)
        raw_budget_fraction = float(mac_cfg.get("adaptive_raw_budget_fraction", 0.90))
        allowed_raw_budget_us = int(raw_budget_fraction * beacon_interval_us)

        return {
            "beacon_interval_us": beacon_interval_us,
            "allowed_raw_budget_us": allowed_raw_budget_us,
            "raw_budget_fraction_ppm": int(raw_budget_fraction * 1_000_000),
        }

    def _validate_total_raw_budget(
        self,
        *,
        ...
        """
        Validate the total sequential RAW duration against the configured RAW budget.
        Since configs are placed on a sequential timeline, the strongest check is
        the maximum end time across enabled configs.
        """
        if not configs:
            return True

        budget = self._get_raw_budget_info()
        allowed_raw_budget_us = int(budget["allowed_raw_budget_us"])
        ...
        for rcfg in configs:
            try:
                ...
                start_time_us = int(rcfg.start_time_us)
                duration_us = num_slots * slot_duration_us
                end_us = start_time_us + duration_us

                total_raw_time_us += duration_us
                if end_us > max_end_us:
                    max_end_us = end_us
            except Exception as e:
                self._safe_log(
                    "RAW_BUDGET_VALIDATE_ERR",
                    {"context": context, "err": str(e)},
                )
                return False

        feasible = max_end_us <= allowed_raw_budget_us
        ...
        self._safe_log(
            "RAW_BUDGET_CHECK",
            {
                ...
                "max_end_us": int(max_end_us),
                "total_raw_time_us": int(total_raw_time_us),
                "allowed_raw_budget_us": int(allowed_raw_budget_us),
                "beacon_interval_us": int(budget["beacon_interval_us"]),
                "raw_budget_fraction_ppm": int(budget["raw_budget_fraction_ppm"]),
    ...
    def init_configs(self) -> List[RawConfig]:
        ...
        if isinstance(cfg_list, list) and cfg_list:
            ...
            if built and not self._validate_total_raw_budget(configs=built, context="custom"):
                self._safe_log(
                    "RAW_CONFIG_FALLBACK",
                    {"reason": "custom_configs_exceed_raw_budget"},
                )
                built = []
    ...
    def _build_default_configs(self, max_aid: int) -> List[RawConfig]:
        ...
        total_required_us = raw_num_groups * per_group_time_us

        budget = self._get_raw_budget_info()
        allowed_raw_budget_us = int(budget["allowed_raw_budget_us"])

        if total_required_us > allowed_raw_budget_us:
            self._safe_log(
                "RAW_STATIC_INFEASIBLE",
                {
                    "raw_num_groups": raw_num_groups,
                    "raw_num_slots": raw_num_slots,
                    "raw_slot_duration_us": raw_slot_duration_us,
                    "per_group_time_us": per_group_time_us,
                    "total_required_us": total_required_us,
                    "allowed_raw_budget_us": allowed_raw_budget_us,
                    "beacon_interval_us": int(budget["beacon_interval_us"]),
                    "reason": "static_raw_exceeds_available_beacon_budget",
                },
            )
            return built
        ...
        if built and not self._validate_total_raw_budget(configs=built, context="default"):
            self._safe_log(
                "RAW_STATIC_INVALID_AFTER_BUILD",
                {"reason": "default_configs_exceed_raw_budget"},
            )
            return []
    ...
    def build_dynamic_configs(self, connected_aids: List[int]) -> List[RawConfig]:
        ...
        if out and not self._validate_total_raw_budget(configs=out, context="dynamic_clamped"):
            self._safe_log(
                "RAW_DYNAMIC_CONFIG_INVALID",
                {"reason": "clamped_static_configs_exceed_raw_budget"},
            )
            return []
