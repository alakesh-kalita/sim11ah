...
class MacContext:
    ...
    _dozing: bool = False
    _twt_wake_gen: int = 0
    # True whenever the STA is in an active TWT Service Period (or TWT is
    # not negotiated / not enabled, in which case it stays True — matches
    # pre-TWT always-awake behavior). RAW window membership always takes
    # priority over this (see RawEngine.raw_enter/raw_exit): a STA is never
    # put to sleep during its own assigned RAW opportunity, TWT-negotiated
    # or not.
    _twt_awake: bool = True
    ...
    # One-shot: set to a frame_seq right after that frame's RTS/CTS exchange
    # succeeds, so the immediately-following _tx_attempt sends DATA directly
    # (no new backoff, per standard). Cleared the instant it's consumed, so
    # a later retry of the same frame_seq (after an ACK timeout) correctly
    # goes through RTS/CTS again rather than skipping it.
    _rts_cleared_for_seq: Optional[int] = None
    ...
    # Management frame priority TX queue (AUTH_REQ/RESP, ASSOC_REQ/RESP).
    # Drained before _txq so management exchange precedes data traffic.
    _mgmt_txq: Any = field(default_factory=deque)

    # Callback invoked by DCF _on_success when a management frame TX succeeds.
    # Set by AssocManager to advance the STA/AP state machine on ACK.
    _on_mgmt_ack_cb: Any = None

    # Callback invoked by AssocManager once a STA reaches ASSOCIATED.
    # Set by MacLayer to TwtManager.on_associated, which kicks off TWT
    # setup negotiation (if twt_enable).
    _on_associated_cb: Any = None
    ...
    # ---------------------------------------------------------------------
    _util_samples: Deque[Tuple[float, bool]] = field(default_factory=deque)
    ...
    # ---------------------------------------------------------------------
    # Metrics (set by MacLayer facade after construction)
    # ---------------------------------------------------------------------
