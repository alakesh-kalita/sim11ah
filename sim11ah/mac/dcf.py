...
# Frame types the receiver genuinely ACKs after SIFS (see facade.py's
# recv_up_from_phy: DATA, AUTH, ASSOC_REQ/RESP, TWT_SETUP_REQ/RESP all
# schedule a real _send_ack). The sender must wait for that ACK (or time
# out and retry) for all of them -- previously only DATA did, so a
# management/TWT frame lost to a real PHY-level collision or interference
# (per_drop/collided at the receiver) was silently treated as an instant
# success on the sender's side, with no timeout ever armed to trigger a
# retry. That left the sender (e.g. a STA mid-association) permanently
# and undetectably stuck waiting for a response that will never arrive.
_ACKED_FTYPES = frozenset({
    FrameType.DATA,
    FrameType.AUTH,
    FrameType.ASSOC_REQ,
    FrameType.ASSOC_RESP,
    FrameType.TWT_SETUP_REQ,
    FrameType.TWT_SETUP_RESP,
})
...
class DcfEngine:
    ...
    def can_tx_now(self) -> bool:
        ...
        if not self.ctx.raw_enable:
            return True
        # Unassociated STAs have no RAW slot yet — they use plain DCF
        # for the authentication/association management exchange.
    ...
    def drive(self, *, make_data_frame_cb=None, record_latency_cb=None) -> None:
        ...
        if self.ctx._pending_frame is None:
            # Priority 1: management frames (AUTH / ASSOC exchange)
            mgmt_q = getattr(self.ctx, "_mgmt_txq", None)
            if mgmt_q:
                self.ctx._pending_frame = mgmt_q.popleft()
                self._reset_retry_counters()
                self.ctx._last_pending_pdu = None
            elif self.ctx._txq:
                # Priority 2: data frames — only allowed once associated
                from sim11ah.mac.common import AssocState
                ...
                # "More Data" (802.11 standard semantics, exploited by
                # E-TAROA/raw_policy_etaroa.py): true whenever this isn't
                # the only PDU queued, i.e. the station still has buffered
                # packets after this one is dequeued. Previously always
                # False -- make_data_frame_cb was only ever called with
                # the bare pdu, so the ctrl["more_data"] field existed on
                # MacFrame but no caller ever set it True.
                more_data = len(self.ctx._txq) > 1
                self.ctx._pending_frame = make_data_frame_cb(pdu, more_data=more_data)
    ...
    def _tx_attempt(self, *, make_data_frame_cb=None) -> None:
        ...
        needs_rts = (
            fr.ftype == FrameType.DATA
            and fr.dst != -1
            and fr.size_bytes > self.ctx.rts_threshold_bytes
            and self.ctx._rts_cleared_for_seq != fr.frame_seq
        )
        rts_overhead_s = self._compute_rts_cts_overhead(fr.dst) if needs_rts else 0.0
        ...
        if needs_rts:
            self._send_rts(fr, make_data_frame_cb=make_data_frame_cb)
            return

        if self.ctx._rts_cleared_for_seq == fr.frame_seq:
            self.ctx._rts_cleared_for_seq = None
        ...
        if fr.ftype in _ACKED_FTYPES and fr.dst != -1:
    ...
    # ------------------------------------------------------------------
    # RTS / CTS (threshold-gated, dot11RTSThreshold = rts_threshold_bytes)
    # ------------------------------------------------------------------
    def _send_rts(self, fr: MacFrame, *, make_data_frame_cb=None) -> None:
        """
        Send RTS for the given (already channel-access-won) DATA frame and
        wait for CTS. Called from _tx_attempt once backoff/RAW-fit checks
        have already passed for the full RTS+CTS+DATA+ACK exchange.
        """
        t_rts = self._compute_rts_tx_time(fr.dst)
        t_cts = self._compute_cts_tx_time(fr.dst)
        t_data = self._compute_data_tx_time(fr)
        t_ack = self._compute_ack_tx_time(fr.dst, fr.ctrl.get("mcs", self.ctx.node.phy.default_mode) if fr.ctrl else self.ctx.node.phy.default_mode)
        prop = self._get_prop(fr.dst)

        # NAV duration: protects the rest of the exchange from this RTS onward
        # (SIFS+CTS+SIFS+DATA+SIFS+ACK), IEEE 802.11-2020 Section 9.3.2.5 / 9.7.
        nav_duration_s = (
            self.ctx.sifs + t_cts + prop
            + self.ctx.sifs + t_data + prop
            + self.ctx.sifs + t_ack + prop
        )

        self.ctx._tx_seq_ctr += 1
        rts = MacFrame(
            ftype=FrameType.RTS,
            src=self.ctx.node.node_id,
            dst=fr.dst,
            size_bytes=self.ctx.rts_size_bytes,
            frame_seq=self.ctx.sim.next_frame_seq(),
            tx_seq=self.ctx._tx_seq_ctr,
            retry=0,
            net_pdu=None,
            ctrl={"duration_s": nav_duration_s, "protects_seq": fr.frame_seq},
        )

        self.ctx.state = MacState.TX
        self._inc_stat("tx_rts")
        self._log("TX_RTS", {"protects_seq": fr.frame_seq, "dst": fr.dst}, frame=rts)
        self.ctx.node.phy.send(rts, tx_id=self.ctx.node.node_id, rx_id=fr.dst)

        # Sender sets its own NAV awareness too (mirrors standard behaviour
        # for any node that transmits/overhears RTS or CTS).
        self.ctx._nav_end = max(self.ctx._nav_end, self.ctx.sim.engine.now + t_rts + nav_duration_s)

        self.ctx.state = MacState.WAIT_CTS
        self.ctx._waiting_cts_for_frame_seq = fr.frame_seq
        timeout_s = t_rts + prop + self.ctx.sifs + t_cts + prop + self.ctx.ack_guard
        self._arm_cts_timeout(fr.frame_seq, timeout_s, make_data_frame_cb=make_data_frame_cb)

    def _arm_cts_timeout(self, frame_seq: int, timeout_s: float, *, make_data_frame_cb=None) -> None:
        self.ctx._cts_timer_token += 1
        token = self.ctx._cts_timer_token
        self._log("CTS_TIMEOUT_SET", {"timeout_s": timeout_s, "token": token}, frame=self.ctx._pending_frame)
        self.ctx.sim.engine.schedule_in(
            timeout_s,
            self._schedule_cts_timeout_event,
            frame_seq,
            token,
            make_data_frame_cb=make_data_frame_cb,
            name="DCF_CTS_TIMEOUT",
        )

    def _schedule_cts_timeout_event(self, frame_seq: int, token: int, *, make_data_frame_cb=None) -> None:
        self._cts_timeout(frame_seq, token, make_data_frame_cb=make_data_frame_cb)

    def _cts_timeout(self, frame_seq: int, token: int, *, make_data_frame_cb=None) -> None:
        if token != self.ctx._cts_timer_token:
            return
        if self.ctx.state != MacState.WAIT_CTS:
            return
        if self.ctx._waiting_cts_for_frame_seq != frame_seq:
            return

        self._log("CTS_TIMEOUT", {"frame_seq": frame_seq}, frame=self.ctx._pending_frame)
        self.ctx._waiting_cts_for_frame_seq = None
        self._handle_rts_retry_or_drop(make_data_frame_cb=make_data_frame_cb)

    def handle_cts_rx(self, frame: MacFrame, *, make_data_frame_cb=None) -> None:
        if self.ctx.state != MacState.WAIT_CTS:
            return
        ctrl = frame.ctrl or {}
        cts_for = ctrl.get("cts_for_seq")
        if cts_for is None or self.ctx._waiting_cts_for_frame_seq != cts_for:
            return

        self._inc_stat("tx_cts")
        self._log("CTS_RX", {"cts_for": cts_for}, frame=frame)
        self.ctx._cts_timer_token += 1
        self.ctx._waiting_cts_for_frame_seq = None

        fr = self.ctx._pending_frame
        if fr is None or fr.frame_seq != cts_for:
            self.ctx.state = MacState.IDLE
            self.drive(make_data_frame_cb=make_data_frame_cb)
            return

        # CTS received: send DATA directly after SIFS, no new backoff.
        self.ctx._rts_cleared_for_seq = fr.frame_seq
        self.ctx.sim.engine.schedule_in(
            self.ctx.sifs,
            self._tx_attempt,
            make_data_frame_cb=make_data_frame_cb,
            name="DCF_TX_DATA_AFTER_CTS",
        )

    def _handle_rts_retry_or_drop(self, *, make_data_frame_cb=None) -> None:
        """
        RTS/CTS failure (no CTS received) always counts against the SHORT
        retry limit, regardless of the protected DATA frame's own size —
        IEEE 802.11-2020 Section 10.3.2.5 (dot11ShortRetryCount also covers
        failed RTS transmissions).
        """
        fr = self.ctx._pending_frame
        if fr is None:
            self.ctx.state = MacState.IDLE
        ...
        if self.ctx._short_retry_count >= self.ctx.short_retry_limit:
            net_pdu = fr.net_pdu
            ...
            self._log("DROP", {"reason": "rts_retry_limit", "retry": fr.retry}, frame=fr)
        ...
        self._clear_backoff_state()
        self.ctx.state = MacState.IDLE
        self._log("RTS_RETRY", {"retry": fr.retry, "cw": self.ctx._cw}, frame=fr)
        self.drive(make_data_frame_cb=make_data_frame_cb)
    ...
    def _handle_retry_or_drop(self, *, make_data_frame_cb=None) -> None:
        ...
        if self._retry_limit_reached(fr):
            ...
            # For management frames (AUTH/ASSOC), notify the AssocManager so it
            # can re-queue the frame and keep the association procedure running.
            _mgmt_ftypes = {"AUTH", "ASSOC_REQ", "ASSOC_RESP"}
            if getattr(fr, "ftype", None) in _mgmt_ftypes:
                drop_cb = getattr(self.ctx, "_on_mgmt_drop_cb", None)
                if callable(drop_cb):
                    try:
                        drop_cb(fr)
                    except Exception:
    ...
    def _on_success(self, *, make_data_frame_cb=None) -> None:
        ...
        # Notify AssocManager when a management frame exchange completes
        _mgmt_ftypes = {"AUTH", "ASSOC_REQ", "ASSOC_RESP"}
        if getattr(fr, "ftype", None) in _mgmt_ftypes:
            cb = getattr(self.ctx, "_on_mgmt_ack_cb", None)
            if callable(cb):
                try:
                    cb(fr)
                except Exception:
    ...
    def _sample_utilization(self) -> None:
        ...
        while self.ctx._util_samples and self.ctx._util_samples[0][0] < cutoff:
            self.ctx._util_samples.popleft()
    ...
    def _compute_rts_tx_time(self, dst: int) -> float:
        if self.ctx.sim.topology is None:
            return (self.ctx.rts_size_bytes * 8) / 65_000.0
        try:
            lk = self.ctx.sim.topology.get_link(self.ctx.node.node_id, dst)
            rate = int(self.ctx.node.phy.mode_table.get(self.ctx.node.phy.control_mode, lk.rate_bps))
        except Exception:
            return (self.ctx.rts_size_bytes * 8) / 65_000.0
        stub = MacFrame(ftype=FrameType.RTS, src=0, dst=0, size_bytes=self.ctx.rts_size_bytes, frame_seq=0, tx_seq=0)
        return float(self.ctx.node.phy.compute_tx_duration(stub, rate))

    def _compute_cts_tx_time(self, dst: int) -> float:
        if self.ctx.sim.topology is None:
            return (self.ctx.cts_size_bytes * 8) / 65_000.0
        try:
            lk = self.ctx.sim.topology.get_link(self.ctx.node.node_id, dst)
            rate = int(self.ctx.node.phy.mode_table.get(self.ctx.node.phy.control_mode, lk.rate_bps))
        except Exception:
            return (self.ctx.cts_size_bytes * 8) / 65_000.0
        stub = MacFrame(ftype=FrameType.CTS, src=0, dst=0, size_bytes=self.ctx.cts_size_bytes, frame_seq=0, tx_seq=0)
        return float(self.ctx.node.phy.compute_tx_duration(stub, rate))

    def _compute_rts_cts_overhead(self, dst: int) -> float:
        """Extra airtime an RTS/CTS-protected exchange needs beyond plain
        DATA+ACK: RTS + prop + SIFS + CTS + prop + SIFS."""
        prop = self._get_prop(dst)
        return self._compute_rts_tx_time(dst) + prop + self.ctx.sifs + self._compute_cts_tx_time(dst) + prop + self.ctx.sifs
