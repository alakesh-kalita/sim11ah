...
class SimLogger:
    ...
    def log_count(self) -> int:
        """Current total record count -- O(1) (no copy), for a caller that
        just needs to know how far the log has grown, e.g. to detect a
        reset/replace (fewer records now than last checked) before
        calling entries_from."""
        with self._lock:
            return len(self._logs)

    def entries_from(self, start: int) -> List[Dict[str, Any]]:
        """Slice from an arbitrary caller-owned index -- independent of
        the shared read cursor new_entries()/mark_read() use (that cursor
        also drives the notebook live-log viewer; a second consumer
        advancing it via mark_read() would make entries disappear from
        the viewer's own "show new" before it ever saw them). Lets a
        caller (e.g. Dashboard._compute_metrics) maintain its own
        incremental "have I processed this" pointer without stepping on
        that. O(len(_logs) - start): slices the tail directly under the
        lock, unlike the `.logs` property (list(self._logs), a full-list
        copy every call regardless of how much of it the caller actually
        needs) or range_records (which also copies the whole list before
        slicing it)."""
        with self._lock:
            return list(self._logs[start:])
