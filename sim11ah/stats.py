...
class SimStats:
    ...
    # (gen_time, recv_time) per source node, in delivery order — used to
    # compute measured Age-of-Information via the sawtooth-area method.
    aoi_records: Dict[int, List] = field(default_factory=lambda: defaultdict(list))
    ...
    # -----------------------------
    # Association timing (populated by AssocManager per STA)
    # -----------------------------
    assoc_total_time: Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    assoc_scan_time:  Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    assoc_auth_time:  Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    assoc_req_time:   Dict[int, float] = field(default_factory=lambda: defaultdict(float))

    # -----------------------------
    # Energy (populated by EnergyModel.finalize)
    # -----------------------------
    energy_tx_j:    Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    energy_rx_j:    Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    energy_idle_j:  Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    energy_sleep_j: Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    energy_total_j: Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    energy_retx_j:  Dict[int, float] = field(default_factory=lambda: defaultdict(float))
    tx_retries_per_node: Dict[int, int] = field(default_factory=lambda: defaultdict(int))
