...
from sim11ah.energy import EnergyModel
...
class Simulator:
    ...
    def finalize(self) -> None:
        ...
        # Aggregate per-node MAC ctx._stats RAW fit counters into global stats
        # (raw.py writes only into ctx._stats; sim.stats fields are never written directly)
        for n in self.nodes.values():
            ctx = getattr(getattr(n, "mac", None), "ctx", None)
            if ctx is None:
                continue
            cs = getattr(ctx, "_stats", {})
            self.stats.raw_fit_pass    += int(cs.get("raw_fit_pass", 0))
            self.stats.raw_fit_fail    += int(cs.get("raw_fit_fail", 0))
            self.stats.raw_fit_blocked += int(cs.get("raw_fit_blocked", 0))

        # Finalise energy model — writes per-node breakdowns into sim.stats
        self.energy.finalize(self.stats.sim_time, self.stats)
